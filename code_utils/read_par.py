...
def read_par(par_path="./Par"):
    ...
    t = nxt_tokens(it)
    updcov, iudc, burnin, sample, skip = map(float, t[:5])
    ...
    for _ in range(ndata):
        ...
        if cindex == 'RPV':
            forward_fn = partial(forward_PVswd, dperi=t, wave='rayleigh', mode=0)
        elif cindex == 'LPV':
            forward_fn = partial(forward_PVswd, dperi=t, wave='love', mode=0)
        elif cindex == 'RGV':
            forward_fn = partial(forward_GVswd, dperi=t, wave='rayleigh', mode=0)
        elif cindex == 'LGV':
            forward_fn = partial(forward_GVswd, dperi=t, wave='love', mode=0)
        elif cindex == 'ELL':
            forward_fn = partial(forward_ell, dperi=t, wave='rayleigh', mode=0)
